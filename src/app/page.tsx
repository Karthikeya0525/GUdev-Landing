      ...
      <AnimatePresence mode="wait">
        {step === 'input' && (
          ...
          >
            ...
            <section id="pricing" className="py-32 px-6 bg-[#F8FAFC]">
               <div className="max-w-4xl mx-auto text-center mb-24">
                  <h2 className="text-5xl md:text-7xl font-black tracking-tight mb-6 text-gray-900">Choose your scale.</h2>
                  <p className="text-gray-500 text-xl font-medium">Invest in the infrastructure that builds your future.</p>
               </div>
               <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10">
                 {[
                   { name: "Founders", price: "$199", desc: "For single visionary founders.", features: ["3 Active Projects", "Standard AI Engine", "Web Preview", "Community Support"] },
                   { name: "Startup Elite", price: "$599", desc: "For fast-scaling product teams.", pop: true, features: ["Unlimited Projects", "Premium 9-Stage Engine", "AR Gesture Previews", "Dedicated AI Architect", "Priority Rendering"] },
                   { name: "Enterprise", price: "$2,499", desc: "Full-scale agentic automation.", features: ["Custom Agent Training", "White-label Export", "On-premise LLM Setup", "24/7 Concierge Service", "SLA Guarantee"] }
                 ].map((p, i) => (
                   <div key={i} className={`p-12 rounded-[3rem] border-2 transition-all duration-500 ${p.pop ? 'bg-gray-900 text-white border-gray-800 shadow-[0_40px_80px_rgba(0,0,0,0.15)] scale-105 z-10' : 'bg-white border-gray-100 shadow-sm hover:shadow-xl hover:border-indigo-100'} relative flex flex-col`}>
                     {p.pop && <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-indigo-500 text-white text-[10px] font-black px-6 py-2 rounded-full tracking-[0.2em]">MOST POPULAR</div>}
                     <h3 className="text-2xl font-black mb-2">{p.name}</h3>
                     <p className={p.pop ? "text-gray-400 text-sm mb-10 font-medium" : "text-gray-500 text-sm mb-10 font-medium"}>{p.desc}</p>
                     <div className="text-5xl font-black mb-10 flex items-baseline">
                        {p.price}
                        <span className="text-lg font-bold text-gray-500 ml-2">/mo</span>
                     </div>
                     
                     <div className="flex-1 space-y-4 mb-12">
                        {p.features.map((f, fi) => (
                           <div key={fi} className="flex items-center gap-3 text-sm font-bold">
                              <CheckCircle2 className={p.pop ? "text-indigo-400" : "text-emerald-500"} size={18} />
                              <span>{f}</span>
                           </div>
                        ))}
                     </div>

                     <button className={`w-full py-5 rounded-2xl font-black tracking-widest transition-all text-sm ${p.pop ? 'bg-indigo-500 text-white hover:bg-indigo-600 shadow-lg shadow-indigo-500/20' : 'bg-gray-900 text-white hover:bg-black'}`}>
                       ACQUIRE LICENSE
                     </button>
                   </div>
                 ))}
